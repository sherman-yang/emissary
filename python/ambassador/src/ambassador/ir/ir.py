...
class IR:
    ...
    def __init__(
        ...
        udp_listeners = (
            listener for listener in self.listeners.values() if listener.socket_protocol == "UDP"
        )
    ...
    def add_mapping(self, aconf: Config, mapping: IRBaseMapping) -> Optional[IRBaseMappingGroup]:
        ...
        if mapping.is_active():
            if mapping.group_id not in self.groups:
                ...
                existing_mapping_cache_keys = [
                    group_mapping["_cache_key"]
                    for group_mapping in group["mappings"]
                    if "_cache_key" in group_mapping
                ]
                if mapping["_cache_key"] in existing_mapping_cache_keys:
                    self.logger.debug(
                        f"IR: _cache_key for {mapping.name} is {mapping['_cache_key']} already exists in a group."
                    )
                else:
